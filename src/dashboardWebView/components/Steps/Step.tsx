      ...
      <>
        {status === Status.NotStarted && (
          <span className="h-9 flex items-center" aria-hidden="true">
            <span
              className={`relative z-10 w-8 h-8 flex items-center justify-center bg-white border-2 border-gray-300 rounded-full ${
                onClick ? 'group-hover:text-gray-400' : ''
              }`}
            >
              <span
                className={`h-2.5 w-2.5 bg-transparent rounded-full ${
                  onClick ? 'group-hover:bg-gray-300' : ''
                }`}
              />
            </span>
          </span>
        ...
        <span className="ml-4 min-w-0 flex flex-col">
          <span className="text-xs font-semibold tracking-wide uppercase text-vulcan-500 dark:text-whisper-500">
            {name}
          </span>
  ...
  return (
    <>
      {showLine ? (
        <div
          className={`-ml-px absolute mt-0.5 top-4 left-4 w-0.5 h-full ${
            status === Status.Completed ? 'bg-teal-600' : 'bg-gray-300'
          }`}
          aria-hidden="true"
        />
      ) : null}
      ...
      {onClick ? (
        ...
        <div className="relative flex items-start group text-left">{renderChildren()}</div>
      )}
    </>
  );
};
